...
struct AuctionTab: View {
    ...
    private var filterBar: some View {
        HStack(spacing: 6) {
            ...
            if showLive {
                FilterChipMenu(
                    label: "Country", isActive: !liveSelectedCountries.isEmpty,
                    options: countryFilters, selected: liveSelectedCountries
                ) { toggle($0, in: liveSelectedCountries) { liveSelectedCountries = $0 } }

                FilterChipMenu(
                    label: "Type", isActive: !liveSelectedTypes.isEmpty,
                    options: typeFilters, selected: liveSelectedTypes
                ) { toggle($0, in: liveSelectedTypes) { liveSelectedTypes = $0 } }

                FilterChipMenu(
                    label: "Region", isActive: !liveSelectedRegions.isEmpty,
                    options: regionFilters, selected: liveSelectedRegions
                ) { toggle($0, in: liveSelectedRegions) { liveSelectedRegions = $0 } }

                FilterChipMenu(
                    label: "Rating", isActive: !liveSelectedRating.isEmpty,
                    options: ["3 Stars", "3+ Stars", "4 Stars"],
                    selected: liveSelectedRating.isEmpty ? [] : [liveSelectedRating],
                    displayLabel: { ratingLabel(for: $0) }
                ) { liveSelectedRating = liveSelectedRating == $0 ? "" : $0 }
                ...
                FilterChipMenu(
                    label: "Country", isActive: !auctionSelectedCountries.isEmpty,
                    options: countryFilters, selected: auctionSelectedCountries
                ) { toggle($0, in: auctionSelectedCountries) { auctionSelectedCountries = $0 } }

                FilterChipMenu(
                    label: "Type", isActive: !auctionSelectedTypes.isEmpty,
                    options: typeFilters, selected: auctionSelectedTypes
                ) { toggle($0, in: auctionSelectedTypes) { auctionSelectedTypes = $0 } }

                FilterChipMenu(
                    label: "Region", isActive: !auctionSelectedRegions.isEmpty,
                    options: regionFilters, selected: auctionSelectedRegions
